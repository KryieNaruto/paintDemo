...
阶段 agent 在 `.cursor/agents/`：`task-claim` / `task-plan` / `task-execute` / `task-test` / `task-finish`（以及 `task-finish` 内部用的 `task-review`）。

## 一轮

1. **看可做任务**：`python .exec/taskline.py available`（只读，别拿 `claim` 探路）。读到可领任务列表。
2. **决定派几个**：默认一次 **1 个**（每个任务一个 worktree + 一套 agent）。人点名 `--task <ID>` 就只做那个；没任务可领就报「当前无可领任务」停下。
3. **派任务**：对要做的任务，**按顺序**派 5 个阶段 agent（用 `Task` 工具，`subagent_type` 选对应角色），阶段间把上一阶段的返回值原样传给下一阶段：
...
- **等待不轮询**：派 agent 一律**前台阻塞**（`run_in_background: false`；不要 `environment: cloud`）。要并行就同一条消息发多个 `Task` 调用。有未终态 agent 就保持等待，不要 `sleep` / `AwaitShell` 轮询。
