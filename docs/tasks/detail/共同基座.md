...
| 内部 L1 | engine、ring_buffer、stroke_predictor（白盒移植 Ink Stroke Modeler） | `core/` |
...
- **任何 UI**：`ui/` `app/` Compose / ImGui / GLFW 窗口（输入平滑已下沉到 SDK 内核 `core/stroke_predictor`）。
...
| B1-1 | 线1-内部 | 内部类型 + IPaintKernel/IRenderBackend + Null 桩，host 可编译 | E0-1 |
| B1-3 | 线1-引擎 | engine 三线程 + ring_buffer SPSC 单测 | E0-3 |
| B1-4 | 线1-CAPI | `sdk_api` C API 封装 engine+Null；未实现项返回错误码 | B1-3 |
| B1-5 | 线1-预测 | 白盒移植 Ink Stroke Modeler 平滑预测 → `core/stroke_predictor` + host 单测 | B1-1 |
...
`android-arm64` **构建验证**在 E0-3（preset 含 `android-arm64`）完成后做。B5-1 无 NDK 时 host ctest 仍须通过。G0-1 无依赖。
...
## B1-5 · 白盒移植 Ink Stroke Modeler → core/stroke_predictor
...
**验收**：`StrokeModeler::Update` 输入原始点流 → 平滑后点流；`StrokeModeler::Predict` 产出预测点带 `isPredicted` 标记；真实点覆盖关系可断言；固定步长欧拉积分（无随机）确定性可测。不实现 GPU 重合成。
...
- E0-3（`android-arm64` preset）满足时编出 `.so`。
