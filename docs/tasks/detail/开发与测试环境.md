...
- Windows 编 `dgc_paint.dll` 的核对表（【人工】）。
...
| E0-4 | 线0-开发环境 | 【人工】Windows host：VS C++ 桌面 + LunarG Vulkan SDK，用于 `dgc_paint.dll` | — | 是 |
...
## E0-4 · 【人工】Windows 编 DLL

**目标**：Windows 开发者能编 `dgc_paint.dll`，不是在 Linux 云环境装 VS。

**产出**

- `docs/env/windows-host.md`：VS「使用 C++ 的桌面开发」+ CMake tools + Ninja；[LunarG Vulkan SDK](https://vulkan.lunarg.com/)。
- 核对表：打开仓库根目录识别 presets（presets 由 B1-2 提供）。

**验收**

- 覆盖编 DLL 所需工作负载；人工勾选即通过。
