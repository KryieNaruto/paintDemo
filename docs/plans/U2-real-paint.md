...
> 说明：`headless.cpp` 固定笔迹为 (100,100) 起、20 步 (20,10) 递增的黑色斜线（seed=42），B3-1 真实内核下必产生黑色像素；无笔迹时 dark=0 → 断言 FAIL。此断言同时是「B3-1 真实内核已生效」的机器可验证据。
> **SDK 背景事实（rev3 实测）**：`dgcClear` 丢弃颜色参数、恒清纯白——review 对真实 B3-1 输出实测 bg=`(255,255,255)`、dark=`973`（>50），与解码器一致。故背景自检**不硬编码常量**、动态导出（见 Step 3 脚本），仅打印 diagnostic。
...
bash tests/smoke.sh
```
Expected: headless 返回 0，PNG 存在且含黑色笔迹像素（颜色数 > 1）；smoke 打印 `PASS: ... stroke pixels=N`（真实内核实测 N=973）。

**门自检（区分度验证，防假绿/假红）**：实现期必须确认门对「真实内核输出」判 PASS、对「空笔迹输出」判 FAIL：
```bash
# ① 当前（9e6eefb 真实内核）跑 smoke → 必须 PASS（N>50，实测 N=973）
bash tests/smoke.sh
# ② 反例：临时把 headless.cpp 的 dgcBeginStroke 之后循环改为空（0 步），重建后跑 smoke
#    预期 dark=0 → FAIL（若 smoke 对空笔迹仍 PASS 说明门失效，禁止进入测试门）
```
> 交叉验证（可选）：本机 PIL 已装（review 实测 10.2.0），可用 `PIL.Image` 复核解码结果与手写解码器一致（review 已对真实输出做过：手写=973、PIL 一致）。
...
- **审阅打回修订（rev1：73.75 → rev2 → rev3）**：
  - F1（rev1 技术可行性）：smoke.sh 原先不解码 scanline 滤波（stb 默认自适应滤波 {Sub/Up/Paeth}，0 行 filter 0），把滤波残差当像素 → 含笔迹也判 FAIL。已重写为滤波感知解码（None/Sub/Up/Average/Paeth 逐行还原）。✓
  - F2（rev2 技术可行性/可测试性）：**背景自检前提错误**——`dgcClear` 在 `dgc_paint_c_api.cpp:232` 丢弃颜色参数、`vk_backend.cpp:815` 硬编码清纯白，真实输出 bg=`(255,255,255)` 而非纸白 `(245,242,232)`；硬编码纸白自检会假红。已改为**动态导出背景（四角众数）+ 自检降级为 diagnostic 打印**，dark 判定独立判真。rev2 实测真实输出 `dark=973`（>50 → PASS）、PIL 交叉验证一致。✓
  - F3（风险）：本计划先前未识别「PNG 滤波解码」与「dgcClear 恒清白」两个直接击穿主证据/假红的风险。已在脚本注释 + Self-Review 显式记录两条防线（滤波感知解码 + 动态背景/无硬编码常量）。✓
  - F4（可测试性）：门加「区分度自检」步骤——真实内核跑应 PASS（实测 N=973）、空笔迹反例跑应 FAIL，确认门可判真后才进测试门，防假绿/假红。✓
  - 可选加固（review 建议，非阻塞）：抽样步长 3 依赖默认笔刷 ~10px 半径，后续调小笔刷可能漏检；届时可加密抽样或改进程内 `dgcReadbackPixels` 断言。
