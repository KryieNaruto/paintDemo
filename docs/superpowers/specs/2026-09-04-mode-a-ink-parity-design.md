...
## 8. A8-3 Phase 0 验证结论（候选①host 核验证伪；候选②真机核验：瓶颈在 Compose/vsync 地板，
   §5.1 降低节流被真机数据证伪为无效改动，不采纳；验收标准口径核对见 §8.3；执行偏离说明见
   §8.5）
...
### 8.5 执行偏离说明：计划内 `LatencyMetrics.kt`（`LatencyProbe` p50/p99）+ `PaintScreen.kt`
   readMs 累积 HUD 未实现（主动判定非必要，非遗漏）

`docs/plans/A8-3.md` §6/§10 原定新增/改动三处（`app/src/main/java/com/dgcamp/paint/ui/
LatencyMetrics.kt` 给 `LatencyProbe` 补充 p50/p99、对应 `LatencyMetricsTest.kt` TDD 用例、
`PaintScreen.kt` 把单点 `readMs` float 改成累积进 `FrameTimeAccumulator` 并在 HUD 展示
均值/p50/p99），理由写的是「Phase 0 §3.2 测量需要更稳的统计量，非单点抖动数字」。

**实际未实现**——`paint-android` `task/A8-3` 分支对这三个文件零改动。这里如实记录为什么：

1. §8.2 记录的真机会话里，候选②判定实际依赖的两组数字都**不是临界值/抖动值**，而是明确
   的类别性（categorical）结果：`lagProbe`（输入→读回）6ms→12ms 是成倍的差异，
   `drawLagProbe`（输入→上屏，验收标准「body 拖后 ≤~1 帧」实际依赖的那个数字）16ms→16ms
   是完全不变。两者的判定结论（「降低节流零收益 + 读回侧有副作用」「瓶颈在 Compose/vsync
   地板不在节流值」）不依赖这两个数字精确到什么置信区间——无论用单点读数还是 p50/p99
   分布，6ms 与 12ms 的类别差异、16ms 与 16ms 的类别相同都不会因为补上分位数统计而改变
   方向。
2. p50/p99 统计量的价值场景是"数字接近临界阈值、抖动大到单点读数可能把判定带偏"（如
   §5.1 原计划设想的"fps 是否卡在 60 门槛线附近"这种场景）——本次两组关键数字都清楚地
   落在同一量级/类别里，不属于这种场景，补统计量只会提升证据的"精细度"，不会改变"结论
   方向"，对已经做出的候选②判定没有增量信息。
3. **这是执行阶段主动判定该计划项在本次真机会话的具体数据格局下非必要后跳过，不是遗漏**
   ——跳过前已经把该计划项的价值边界想清楚（见上两条），跳过后也如实记录在此，而不是在
   test-review 打回后才补丁式地"解释一下"掩盖没做的事实。若未来某次真机会话里 `readMs`
   或其他延迟数字确实落在临界区间、单点读数不足以支撑判定，应该按原计划把这三个文件的
   改动补上，届时不应该再复用本节的"非必要"结论。

不涉及生产代码/SDK 侧改动；本次仅为文档补充，未触碰真机、未改 `ReadbackScheduler.kt` 或
任何 `paint-android`/`demo` 生产代码。
