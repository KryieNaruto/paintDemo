...
struct StrokeModelParams {
    ...
    // 注（bugfix Fix B 校准依据）：原默认 400.0/40.0 对应 ωn=√K=20 rad/s → 临界阻尼沉降
    //   ~200ms，相对输入节奏（每点 5.5~10ms）过慢，弹簧把笔迹垂直分量全部抹平 → 激活
    //   modeler 的笔画成水平杠/消失（Bug #3 复现 2）。上调配对 k=40000/c=400（ωn=200
    //   rad/s，临界阻尼 ζ=C/(2√K)=1）后，沉降 ~2-3 个输入步长，弹簧跟住笔迹主体而仍
    //   平滑高频抖动；经 tests/test_modeler_stroke_renders 校准，默认参数下输出保持输入
    //   笔迹形态（横向跨度 ≥80%、纵向极差 ≥50%、墨迹 ≥ 无 modeler 基线 50%），且
    //   test_stroke_predictor 的「平滑降方差」断言不回归（wobble 低通已先压制高频分量）。
    //   调大平滑参数（增大 spring_mass_constant 之外的「粘滞」由 drag 承担）仍更平滑。
    // 单位：1/秒²（1/s²）。
    float spring_mass_constant = 40000.0f;
    ...
    // 单位：1/秒（1/s）。
    float spring_drag_constant = 400.0f;
